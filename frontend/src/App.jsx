...
function App() {
  ...
  return (
    <div className="app">
      ...
      {page === 'dashboard' ? <Dashboard t={t}/> : <CropDetection t={t}/>}
    </div>
  );
}

function Navbar({lang, setLang, t, page, setPage}) {
  return (
    <nav className="navbar">
      ...
      <div className="nav-links">
        <a href="#dashboard" onClick={() => setPage('dashboard')} style={{color: page==='dashboard' ? '#00c853' : ''}}>{t.dashboard}</a>
        <a href="#crops">{t.cropHealth}</a>
        <a href="#alerts">{t.alerts}</a>
        <a href="#analytics">{t.analytics}</a>
        <a href="#detect" onClick={() => setPage('detect')} style={{color: page==='detect' ? '#00c853' : ''}}>🔍 {t.cropDetection}</a>
      </div>
...
function CropDetection({t}) {
  ...
  const drawBoxes = (detections, imgElement) => {
    ...
    detections.forEach((det) => {
      ...
      const color = det.confidence >= 0.7 ? '#ff5252' : det.confidence >= 0.4 ? '#ffd600' : '#00c853';
  ...
  const handleDetect = async () => {
    ...
    try {
      ...
      const data = await res.json();
      setResult(data);
      // Draw boxes after state update
      setTimeout(() => {
        if (imageRef.current) {
          drawBoxes(data.detections, imageRef.current);
        }
      }, 100);
    } catch (err) {
  ...
  const getSeverityColor = (confidence) => {
    if (confidence >= 0.7) return '#ff5252';
    if (confidence >= 0.4) return '#ffd600';
    return '#00c853';
  };

  return (
    <div className="dashboard">
      <h1 className="dashboard-title">🔍 {t.cropDetection}</h1>
      <p className="dashboard-sub">Upload a crop image — YOLOv8 Nano AI will detect diseases and objects</p>

      <div className="control-section">
        <h2 className="chart-title">📤 Upload Image</h2>
        <p className="chart-sub">Supports JPG, PNG — drone field photos or crop close-ups</p>

        <div style={{display:'flex', flexDirection:'column', alignItems:'center', gap:'20px', padding:'30px'}}>
          <label style={{
          ...
          {preview && (
            <div style={{position:'relative', display:'inline-block'}}>
              ...
              />
              <p style={{color:'#8899bb', marginTop:'8px', fontSize:'14px', textAlign:'center'}}>{file?.name}</p>
            </div>
          )}
          ...
          <button
            ...
            className="control-btn btn-mission"
            style={{padding:'14px 40px', fontSize:'16px', opacity: (!file || loading) ? 0.5 : 1}}
          >
            {loading ? '⏳ Analyzing...' : '🤖 Run YOLOv8 Detection'}
          </button>
        </div>

        {error && (
          <div className="disease-alert" style={{borderColor:'#ff5252', background:'rgba(255,82,82,0.08)'}}>
            <span>❌</span>
        ...
        {result && (
          <div style={{padding:'20px'}}>
            <h2 className="chart-title">📊 Detection Results</h2>
            <p className="chart-sub">{result.count} object(s) detected in {result.filename}</p>

            {result.count === 0 ? (
              <div className="disease-alert" style={{borderColor:'#ffd600', background:'rgba(255,214,0,0.08)'}}>
                <span>⚠️</span>
                <div><div className="alert-title">No objects detected — try a clearer image</div></div>
              </div>
            ) : (
              <div className="alerts-list" style={{marginTop:'16px'}}>
                {result.detections.map((det, i) => (
                  <div key={i} className="alert-item" style={{
                    borderColor: getSeverityColor(det.confidence),
                    background: 'rgba(0,200,83,0.05)'
                  }}>
                    <div className="alert-left">
                      ...
                      <div>
                        <div className="alert-top">
                          <span className="alert-label" style={{color: getSeverityColor(det.confidence)}}>
                            {Math.round(det.confidence * 100)}% confidence
                          </span>
                          <span className="alert-zone">{det.label.toUpperCase()}</span>
                        </div>
